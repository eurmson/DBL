#![allow(unused)]
use super::algorithm_hiding::UniqueId;
...
struct RGData{
    ...
    commit_map: HashMap<UniqueId, PathBuf>,
}
...
pub fn action_handler(command: String, repo_name: String, file_names: Vec<PathBuf>, branch_name: String) -> std::result::Result<String, String>{
    ...
    if command == "add"{
        ...
        rg.remove_files(vec![PathBuf::from("file_2.txt")]);
        println!("{:?}", rg.curr_rev);
        println!("{:?}", rg.heads.get("main"));
    ...
    if command == "commit"{
        rg.add_graph_node(branch_name.clone());
        // TODO: Add call to save file fn in file_log to get file ID associated to snapshot saved 
...
pub fn tests () {
    ...
    let file_names = vec![PathBuf::from("file_1.txt"), PathBuf::from("file_2.txt"), PathBuf::from("file_3.txt")]; 
    let mut path_buf = PathBuf::new();
    path_buf.push("file_1.txt");
    let path = path_buf.as_path(); // A reference to path is what the file log module takes anyways
    println!("{:?}", path);
